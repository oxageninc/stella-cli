...
The top-level `tools` section switches built-in tools that are **off by default**. Today
that is exactly one tool — raw `bash`:
...
`build_project`, `run_script`, the process tools) instead. The values are the strings
`"on"`/`"off"` — a bare boolean or a typo is a hard, named parse error rather than a
silent ignore. Merges last-scope-wins, so a project can switch bash on for a repo whose
workflow needs it while your user scope keeps it off. See
[Built-in tools](/docs/agent-tools) for what registers when.
