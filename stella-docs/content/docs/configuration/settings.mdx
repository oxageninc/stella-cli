...
`settings.json` is Stella's configuration file. It has five top-level sections, each with
...
| [`tools`](#the-tools-section) | Built-in tool switches (today: `bash`) | last scope wins |
...
(`~/.config/stella/settings.json`) it is the recommended way to, for example, route a Z.ai
...
separately in `~/.config/stella/web_auth.toml`.

The values are the strings `"on"`/`"off"` — a bare boolean or a typo is a hard, named
parse error rather than a silent ignore. Each field merges last-scope-wins independently,
so a project can switch `web` on for a repo whose workflow needs it while your user scope
keeps it off. See [Built-in tools](/docs/agent-tools) for what registers when.
