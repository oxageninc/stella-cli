...
| Goal mode | `stella goal "…"` | An outcome, not a task list | An independent [judge](#outcome-driven-goal-mode), in rounds |
...
`stella goal` doesn't take a task — it takes an **objective**, and turns
Stella loose on it until the goal is actually met, as judged by an
independent model rather than the worker's own say-so.
