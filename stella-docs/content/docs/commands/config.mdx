...
<Callout type="info">
`settings.json` is merged field-by-field across three scopes — project (`<workspace>/.stella/settings.json`), org-managed, and user (`~/.stella/settings.json`) — highest precedence first. `stella config` shows the merged, effective values.
</Callout>
