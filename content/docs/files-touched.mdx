...
`reason` string. It is stored verbatim in the audit log so the ledger reads like a
changelog rather than a list of paths. When omitted, Stella fills in a per-operation
...
`stella run --output-format json` includes the full telemetry payload under `files_touched`, so a
script can assert on exactly what a run changed:

```bash
stella run --output-format json "add a --health flag and wire the readiness probe" \
...
```bash
touched=$(stella run --output-format json "$TASK" | jq -r '.files_touched[].path')
...
<Callout type="info">
  ...
  end? The [extension hooks](/docs/extensions) event bus emits `file.created`,
